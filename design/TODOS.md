...
**Blocked: converting Solitaire to Gum's JSON project format (`.gumj`/`.gusj`/`.gucj`/`.gutj`/`.behj`, replacing XML).** Gum `2026.8.3.1` (bumped in `Directory.Packages.props`, along with `gumcli`) ships `convert-to-json`, and `gumcli check` loads the converted JSON project cleanly. But `gumcli pack` on that same version doesn't know about the JSON family yet — it still resolves element dependencies by hardcoded XML extension (`.gusx`/`.gucx`/`.gutx`/`.behx`) and reports every referenced element as `missing` when packing a `.gumj` project. Since Solitaire ships bundled by default, this would break the shipped build, so the conversion was reverted (Solitaire stays on XML) pending an upstream `gumcli pack` fix. Re-attempt once a newer `gumcli`/`Gum.*` release resolves JSON-family dependencies during pack — re-run `convert-to-json`, `gumcli check`, then `gumcli pack` on the result before trusting it, then measure load-time improvement on Solitaire before/after.
