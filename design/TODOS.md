...
Initial load is slow on the BlazorGL/KNI target when a `.gumx` references many components — fonts, control templates, generated runtime types all compile/initialize on the main thread before the first frame draws. `.gumpkg` bundling (fewer HTTP round-trips) is already Solitaire's default (`Solitaire.GumPackage.targets`, `UseGumPackage` defaults `true`) — nothing to do there.

**Blocked: converting Solitaire to Gum's JSON project format (`.gumj`/`.gusj`/`.gucj`/`.gutj`/`.behj`, replacing XML).** First attempt (Gum `2026.8.3.1`) hit `gumcli pack` resolving element dependencies by hardcoded XML extension — fixed upstream in `gumcli`/`Gum.*` `2026.8.5.1-preview.1` (Gum PR #4346). Bumped to that version and re-tried: `convert-to-json`, `gumcli check`, and `gumcli pack` (standalone CLI invocation) all now succeed on the JSON project. But loading the resulting `.gumpkg` **at runtime** still fails — `GumBundleLoader.Resolve` (`GumCommon/Bundle/GumBundleLoader.cs` in the Gum repo) always synthesizes the bundle's internal project entry as `<name>.gumx` regardless of the packed project's actual format, so it looks for `GumProject.gumx` inside a bundle that only contains `GumProject.gumj` and throws `FileNotFoundException`. This is the same class of hardcoded-XML-extension bug as the pack-time one, but in the runtime bundle loader (shipped in `Gum.MonoGame`/`Gum.KNI`, not just the `gumcli` tool) — a separate fix is needed there. Reverted Solitaire to XML again pending that fix. Loose mode (`.gumj` without bundling) already loads correctly at runtime — only the bundle path is broken. Re-attempt once `GumBundleLoader.Resolve` derives the bundle's internal entry name/extension from the bundle's actual manifest instead of assuming `.gumx`; re-verify with a real `dotnet build` (not just standalone `gumcli pack`) followed by actually running the built exe, since `gumcli check`/`pack` succeeding was not sufficient to catch this.

Once unblocked, still need to (a) measure where the remaining load time goes (Gum project load vs. runtime registration via `RegisterRuntimeType` module initializers vs. asset decode), (b) decide whether further fixes are engine-side (lazy/deferred runtime registration, parallel asset decode) or Gum-side (incremental project load), and (c) confirm WASM-specific costs separate from the same load on desktop.
